...
import API from './StorekeeperAPI';
import ItemDetails from './ItemDetail';

const ItemOverview = ({ brandColor }) => {
  ...
  useEffect(() => {
    const fetchSummaryData = async () => {
      try {
        ...
        const data = await API.fetchBranchItemByPharmacy(id);
        setSummaryData(data);
      } catch (error) {
    ...
    <div className="bg-gray-100 p-4 w-full">
      <div className="w-full">
        {!selectedCategory ? (
          ...
          <ItemDetails category={selectedCategory} goBack={() => setSelectedCategory(null)} brandColor={brandColor} />
        )}
      </div>
    </div>
  );
